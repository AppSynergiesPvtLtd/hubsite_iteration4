...
const WheelComponent = ({
  ...
  const drawWheel = () => {
    ...
    ctx.font = "bold 14px Arial";
    ctx.textAlign = "center";
    ctx.fillText(buttonText, size / 2, size / 2 + 5);
  };
  ...
  return (
    <div className="bg-green-500 min-h-[500px] min-w-[650px] p-2" style={{ textAlign: "center", position: "relative", }}>

      {!showReward && !showNextRewards && (
        <div>
          <div className="text-[#0057A1] ">
            <h1 className="text-3xl mb-4">Spin the Reward Roulette!</h1>
            <p className="md:text-xl mt-2 md:w-[640px]">
              Spin the wheel for exciting rewards and discover exclusive offers
              just for you!
            </p>
          </div>
          <div
          ...
          <button
            style={{
              ...
              borderRadius: "5px",
              fontSize: "1rem",
              cursor: isSpinning || (isFinished && isOnlyOnce) ? "not-allowed" : "pointer",
            }}
            onClick={spin}
      ...
      {showReward && (
        <div>
          <h2 className="text-3xl">Your Rewards Await!</h2>
          <div className="reward-display">
            <p className="text-xl">{currentReward} Hubscore</p>
            <div className="bg-white w-[280px] h-[280px] flex justify-center flex-col align-middle m-auto rounded-full">
              ...
              <p className="text-3xl text-[#0057A1">Hubcore</p>

            </div>
            <button className="p-3 md:w-[300px] bg-[#0057A1] rounded-lg text-white text-xl" onClick={handleClaimReward}>Claim</button>
          </div>
        </div>
      )}
      ...
      {showNextRewards && (
        <div>
          ...
          <p>Unlock Additional Gifts with Each Survey You Complete!</p>
          ...
          <button onClick={resetState}>Got it</button>
        </div>
      )}
    </div>
  );
