...
const QuestionsSurvey = () => {
          ...
          fields:
            q.type === 'TEXT'
              ? [
                  {
                    name: 'response',
                    placeholder: t('index.textAnswerPlaceholder'),
                  },
                ] // Use translation key
              : [],
          options:
  ...
  const validateStep = () => {
    ...
    if (
      ...
      stepErrors = { selectedOption: t('index.answerRequired') } // Use translation key
    }
  ...
  if (loading) {
    return <div>{t('index.loading')}</div> // Use translation key
  }
    ...
    return (
      <div className='w-full flex flex-col items-center justify-center min-h-[80vh] text-center gap-6'>
        ...
        <h2 className='text-[#0057A1] text-[28px] md:text-[36px] font-bold'>
          {t('index.surveyCompletedTitle')}
        </h2>
        <p className='text-gray-600 text-lg'>
          {t('index.surveyCompletedMessage', { surveyId })}
        </p>
        <button
