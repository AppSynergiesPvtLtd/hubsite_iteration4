...
const Contactus = () => {
  ...
  const [viewStyle, setViewStyle] = useState(t('contact.tableView'))
  ...
  const dataFields = [
    { key: 'name', label: t('contact.tableHeaders.name') },
  ...
  useEffect(() => {
    dispatch(setTitle(t('contact.title')))
    dispatch(
  ...
  const fetchUsers = async () => {
    ...
    try {
      const payload = {
        sort:
          sortOrder === t('contact.ascending')
            ? 'asc'
            : sortOrder === t('contact.descending')
            ? 'desc'
            : undefined,
  ...
  const deleteMessage = async (id) => {
    try {
      ...
      setAlert({
        show: true,
        message: t('contact.messageDeletedSuccess'),
        type: 'success',
      })
    } catch (error) {
      ...
      setAlert({
        show: true,
        message: t('contact.messageDeletedError'),
        type: 'error',
      })
    }
  }
    ...
    <div className='p-4'>
      ...
      <div className='flex flex-wrap items-center justify-between mb-6 gap-4'>
        ...
        <div className='flex flex-wrap items-center gap-4 w-full sm:w-auto'>
          ...
          >
            <option>{t('contact.tableView')}</option>
            <option>{t('contact.gridView')}</option>
            <option>{t('contact.listView')}</option>
          </select>
        ...
        <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 z-50'>
          <div className='w-96 bg-white rounded-lg shadow-lg'>
            <div className='flex items-center justify-between p-4 border-b'>
              <h3 className='text-lg font-semibold'>{t('contact.sortTitle')}</h3>
              <button
            ...
            <div className='p-4 space-y-6'>
              <div>
                <h4 className='text-sm font-medium mb-3'>{t('contact.sortByDate')}</h4>
                <div className='space-y-3'>
                  <label className='flex items-center gap-2'>
                    <input
                      type='radio'
                      className='w-4 h-4 border-gray-300 text-blue-600 focus:ring-blue-500'
                      checked={tempSortOrder === t('contact.ascending')}
                      onChange={() => setTempSortOrder(t('contact.ascending'))}
                    />
                    <span className='text-sm text-gray-700'>{t('contact.ascending')}</span>
                  </label>
                  <label className='flex items-center gap-2'>
                    <input
                      type='radio'
                      className='w-4 h-4 border-gray-300 text-blue-600 focus:ring-blue-500'
                      checked={tempSortOrder === t('contact.descending')}
                      onChange={() => setTempSortOrder(t('contact.descending'))}
                    />
                    <span className='text-sm text-gray-700'>{t('contact.descending')}</span>
                  </label>
                </div>
              </div>
            </div>
      ...
      <div>
        {loading ? (
          <div className='flex justify-center items-center h-32'>
            <p className='text-gray-500 text-lg'>{t('contact.loading')}</p>
          </div>
        ) : filteredData.length > 0 ? (
          <>
            {viewStyle === t('contact.tableView') ? (
              <div className='w-full overflow-auto'>
                <table className='w-full text-left'>
                  <thead>
                    <tr className='text-sm text-gray-600 font-medium'>
                      <th className='py-3 px-4'>{t('contact.serialNumber')}</th>
                      {dataFields
                      ...
                      <th className='py-3 px-4'>{t('contact.actions')}</th>
                    </tr>
                  </thead>
                      ...
                      <div className='space-y-2'>
                        ...
                        {visibleFields.includes('email') && (
                          <p className='text-gray-600 break-words'>
                            <b>{t('contact.gridLabels.email')}:</b> {item.email}
                          </p>
                        )}
                        {visibleFields.includes('message') && (
                          <p className='text-gray-600 break-words'>
                            <b>{t('contact.gridLabels.message')}:</b> {item.message}
                          </p>
                        )}
                        {visibleFields.includes('createdAt') && (
                          <p className='text-gray-600 break-words'>
                            <b>{t('contact.gridLabels.created')}:</b>{' '}
                            {new Date(item.createdAt).toLocaleDateString()}
                          </p>
                        )}
          ...
          <div className='flex justify-center items-center h-32'>
            <p className='text-gray-500 text-lg'>{t('contact.noMessages')}</p>
          </div>
        )}
      </div>
