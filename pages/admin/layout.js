...
import { MdDashboard, MdArrowBack, MdAdd, MdRefresh } from 'react-icons/md'
import {
  FaUsers,
  FaPoll,
  FaTasks,
...
const Layout = ({ children, isLoading = false }) => {
    ...
    const sidebarItems = [
      {
        name: 'dashboard',
        label: t('layout.dashboard'), // Use t function
        path: '/admin',
        icon: 'Dashboard',
      },
      {
      ...
      {
        name: 'Survey Taken',
        label: t('layout.surveyTaken'),
        icon: 'Survey Taken',
        subItems: [
          {
            name: 'Profile Survey Completions',
            label: t('layout.profileSurveyCompletions'),
            path: '/admin/survey-taken',
            icon: 'Profile',
          },
          {
            name: 'Live-Survey-Completions',
            label: t('layout.liveSurveyCompletions'),
      ...
      {
        name: 'Manage Surveys',
        label: t('layout.manageSurveys'),
        icon: 'Manage Surveys',
        subItems: [
          {
            name: 'Profiling Surveys',
            label: t('layout.profilingSurvey'),
            path: '/admin/manage-surveys/profiling-survey',
            icon: 'Profiling',
          },
          {
            name: 'Live Survey',
            label: t('layout.liveSurvey'),
            path: '/admin/manage-surveys/live-survey',
            icon: 'LiveSurvey',
          },
          {
            name: 'Onboarding Survey',
            label: t('layout.onboardingSurvey'),
      ...
      {
        name: 'Contact-us',
        label: t('layout.contactUs'),
        path: '/admin/contact-us',
        icon: 'Contact Us',
      },
      {
        name: 'Newsletter',
        label: t('layout.newsletter'),
        path: '/admin/newsletter',
        icon: 'Newsletter',
      },
      {
        name: 'Testimonials',
        label: t('layout.testimonials'),
    ...
    return (
      <div className='poppins flex h-screen'>
        ...
        <div className='flex-1 overflow-auto'>
              ...
              {adminBtn.add && (
                ...
                >
                  ...
                  <span className='hidden md:inline ml-1'>
                    {adminBtn.add.label || t('layout.add')}
                  </span>
                </button>
              )}
              {adminBtn.refresh && (
                ...
                >
                  ...
                  <span className='hidden md:inline'>
                    {adminBtn.refresh.label || t('layout.refresh')}
                  </span>
                </button>
              )}
              {adminBtn.excel && (
                ...
                >
                  ...
                  <span className='hidden md:inline ml-1'>
                    {adminBtn.excel.label || t('layout.generateExcel')}
                  </span>
                </button>
              )}
            </div>
          ...
          <div
            className={`relative mt-6 h-full ${isLoading ? 'opacity-50' : ''}`}
          >
            {isLoading && (
        ...
        {showLogoutModal && (
          <div className='fixed inset-0 flex items-center justify-center z-50'>
            ...
            <div className='relative bg-white p-6 rounded-md shadow-md z-10 w-80'>
              ...
              <p className='mb-6'>{t('layout.logoutConfirmation')}</p>
