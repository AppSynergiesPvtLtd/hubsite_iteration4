...
const Live_Survey_Completions = () => {
    ...
    useEffect(() => {
        dispatch(setTitle(t("surveyCompletions.title")));
        dispatch(
            showExcel({
                label: t("surveyCompletions.generateExcel"),
    ...
    const fetchUsers = async () => {
        ...
        try {
            const response = await axios.get(
                `${API_BASE_URL}/live-survey/usersTakenSurvey/${id}`,
                {
                    headers: {
                        "x-api-key": API_KEY,
        ...
        <div className="p-4 max-w-full">
            ...
            <div className="flex flex-wrap items-center justify-between mb-6 gap-4">
                ...
                <div className="flex-grow relative w-full sm:w-auto">
                    <input
                        type="text"
                        placeholder={t("surveyCompletions.searchPlaceholder")}
                ...
                <div className="flex flex-wrap items-center gap-4 w-full sm:w-auto">
                    ...
                    >
                        <option>{t("surveyCompletions.tableView")}</option>
                        <option>{t("surveyCompletions.gridView")}</option>
                        <option>{t("surveyCompletions.listView")}</option>
                    </select>
                    ...
                    >
                        <option value={10}>{t("surveyCompletions.view10")}</option>
                        <option value={50}>{t("surveyCompletions.view50")}</option>
                        <option value={100}>{t("surveyCompletions.view100")}</option>
                    </select>
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30">
                    <div className="w-96 bg-white rounded-lg shadow-lg p-6">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-lg font-semibold text-gray-800">{t("surveyCompletions.sort")}</h3>
                            <button
                        ...
                        <div className="mb-6">
                            <p className="text-sm text-gray-600 mb-3">{t("surveyCompletions.sortBy")}</p>
                            <div className="flex flex-col gap-3">
                                <label className="flex items-center gap-2">
                                    ...
                                    />
                                    <span className="text-sm text-gray-700">{t("surveyCompletions.ascending")}</span>
                                </label>
                                <label className="flex items-center gap-2">
                                    <input
                                        type="radio"
                                    ...
                                    />
                                    <span className="text-sm text-gray-700">{t("surveyCompletions.descending")}</span>
                                </label>
                            </div>
                        </div>
                        <div className="flex items-center justify-end gap-3">
                            ...
                            >
                                {t("surveyCompletions.clear")}
                            </button>
                            <button
                                onClick={handleApplySort}
                                className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700"
                            >
                                {t("surveyCompletions.apply")}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div className="overflow-auto">
                {loading ? (
                    <div className="flex justify-center items-center h-32">
                        <p className="text-gray-500 text-lg">{t("surveyCompletions.loading")}</p>
                    </div>
                    ...
                    viewStyle === "Table View" ? (
                        <div className="w-[90vw] md:w-full overflow-auto">
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="text-sm text-gray-600 font-medium">
                                        ...
                                        <th className="py-3 px-4">{t("surveyCompletions.name")}</th>
                                        <th className="py-3 px-4">{t("surveyCompletions.email")}</th>
                                        <th className="py-3 px-4">{t("surveyCompletions.phone")}</th>
                                        <th className="py-3 px-4">{t("surveyCompletions.hubCoins")}</th>
                                        <th className="py-3 px-4">{t("surveyCompletions.createdAt")}</th>
                                        <th className="py-3 px-4">{t("surveyCompletions.action")}</th>
                                    </tr>
                                </thead>
                            ...
                            <div className="flex justify-between items-center mt-4">
                                ...
                                >
                                    {t("surveyCompletions.prev")}
                                </button>
                                <span className="text-sm">
                                    {t("surveyCompletions.page")} {currentPage} of {totalPages}
                                </span>
                                <button
                                    onClick={() =>
                                        setCurrentPage((prev) =>
                                ...
                                >
                                    ...
                                    <div className="mt-2">
                                        ...
                                        <p className="text-gray-600 mb-1">
                                            <b>{t("surveyCompletions.email")}:</b> {item.email}
                                        </p>
                                        <p className="text-gray-600 mb-1">
                                            <b>{t("surveyCompletions.phone")}:</b> {item.phone}
                                        </p>
                                        <p className="text-gray-600 mb-1">
                                            <b>{t("surveyCompletions.hubCoins")}:</b> {item.hubCoins}
                                        </p>
                                        <p className="text-gray-600">
                                            <b>{t("surveyCompletions.createdAt")}:</b>{" "}
                                            {new Date(item.createdAt).toLocaleDateString()}
                                        </p>
                                    </div>
                                </div>
                    ...
                    <div className="flex justify-center items-center h-32">
                        <p className="text-gray-500 text-lg">{t("surveyCompletions.noUsers")}</p>
                    </div>
                )}
            </div>
        </div>
