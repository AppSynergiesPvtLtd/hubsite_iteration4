...
const SurveyDetails = () => {
  ...
  useEffect(() => {
    ...
    // Get the survey ID from either route params or query params
    const id = router.query.id || (router.asPath.match(/[?&]id=([^&]+)/) || [])[1];
  ...
  return (
    <div className='flex justify-center p-1 sm:p-4'>
      <div className='w-full  sm:p-6 bg-white md:border rounded-md shadow-md flex flex-col gap-5'>
        {/* Display Survey ID for debugging (can be removed later) */}
        <div className="text-sm text-gray-500">
          Survey ID: {surveyId || 'Not available'}
        </div>
        ...
        <div className='flex flex-col w-full md:w-[90%] items-center'>
          ...
          <button
            ...
            className={`w-[70%] sm:w-[60%] md:w-[40%] lg:w-[30%] xl:w-[20%] p-3 mb-6 ${
              !surveyId || isFetchingError
                ? 'bg-gray-400'
                : 'bg-blue-600 hover:bg-blue-700'
            } text-white font-medium rounded-md`}
            disabled={!surveyId || isFetchingError}
          >
